...
function inputManeger(inputType){
 
  const leftChars = maxChars - nameInput.value.length - messageInput.value.length;
  counter.innerText = `${maxChars - leftChars} / ${maxChars}`;
   if (leftChars < 0){
    inputType.value = inputType.value.slice(0, leftChars);
    const reLeftChars = maxChars - nameInput.value.length - messageInput.value.length;
    counter.innerText = `${maxChars - reLeftChars} / ${maxChars}`;
  }
}
...
amountInput.addEventListener("input", () => {
  ...
  //100NYANCOより少ないとメッセージand入力不可
  if (val.includes(".") || Number.isNaN(amount) || amount < 100) {
    messageInput.disabled = true;
    messageInput.value = "";
    messageInput.maxLength = 0;
    nameInput.disabled = true;
    counter.innerText = "Minimum 100 NYANCO, only natural number";
    return;
  }else if (amount < 1000){
    maxChars = 30;
  } else if (amount < 5000){
...
//小数点不可
  ...
  if (val.includes(".")) {
    amountInput.value = val.split(".")[0];
  }
  */
messageInput.disabled = false;
nameInput.disabled = false;
messageInput.maxLength = maxChars;
nameInput.maxLength = maxChars;
inputManeger(messageInput);
  
});

messageInput.addEventListener("input", () => {
  inputManeger(nameInput);
});
